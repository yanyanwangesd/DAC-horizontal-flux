subroutine captures_and_divides (geometry,network,params,delaunay)
  ...
  ! Then these 2 steps are repeated for Hm/n not equal to 1
  ...
  ! !!! # Y. Wang, Feb. 2022
  ! params%small_divide, small divide option is not used whatever the input is.
  ! should be fixed
  ! !!! # Y. Wang, Feb. 2022
  ...
  !!!!!!!!!!!!! @ Y. Wang, March 2022, write deltachi to txt file and update geometry%chi at the end of this routine
  double precision,dimension(:),allocatable::chi
  double precision deltachi1, deltachi2, Anot
  !!!!!!!!!!!!! @ Y. Wang, March 2022, write deltachi to txt file and update geometry%chi at the end of this routine
  
  !!!!!!!!!!!!! @ Y. Wang to save big capture data
  allocate(contributing_area(geometry%nnode))
  allocate(chi(geometry%nnode))
  !!!!!!!!!!!!! @ Y. Wang to save big capture data
  ...
  do i=1,geometry%nnode
    if (network%receiver(i).ne.0.and.geometry%fix(i).eq.0) then
        	...
        	if (geometry%z(j).gt.geometry%z(i)) then
        	    print*,'Capture check0: flowing uphill - cut river make lake, i lower than j:',geometry%x(i),geometry%y(i),geometry%x(j),geometry%y(j)
                network%receiver(i)=0
                geometry%erosion_rate(i)=0.0d0 
                !geometry%z(j)=geometry%z(i)
            	!geometry%erosion_rate(j)= (geometry%z(i)-geometry%z(j))/params%deltat
...
!                    if (geometry%z(i).lt.geometry%z(j) .and. geometry%fix(j).eq.0 .and. fixprod.ne.1) then
                    if (geometry%z(i).lt.geometry%z(j) .and. geometry%fix(j).eq.0 .and. fixprod.eq.0) then                 
                ...
                if(network%receiver(j).ne.i.and.network%receiver(i).ne.j) then ! only for connections that are not part of the drainage network
                    fixprod= geometry%fix(i)*geometry%fix(j)
                       if (geometry%z(i).lt.geometry%z(j) .and. geometry%fix(j).eq.0 .and. fixprod.eq.0) then                  
                        ...
                        if (zt.lt.geometry%z(j)) then ! first case : capture
                            if (orig_receiver(j).eq.i ) then
                             !print*, geometry%x(i), geometry%y(i), 'is caturing its former donor', geometry%x(j), geometry%y(j)
                             !stop
                            endif
                            if (params%capture) then
                                ncapture=ncapture+1
...
1493 continue
        ...
        do k=geometry%nb(i),1,-1 ! loop over all neighbours
            ...
            if(network%receiver(j).ne.i.and.network%receiver(i).ne.j) then ! only for connections that are not part of the drainage network
                if (geometry%z(i).lt.geometry%z(j)  .and. geometry%fix(j).eq.0  ) then ! check that i is lower than j
                    ...
                    ! diffusional hillslope is turned on when divide height of critical slope is higher than diffusional slope
                    if(params%diffusion.and.xc.lt.(params%diffusivity*params%tanthetac/local_erosioni))then
                        ! params%min_tan_head_slope is minimum slope of channel head (used as a threshold to diffusion)
                        zc=max(local_erosioni*xc**2.d0/params%diffusivity,xc*params%min_tan_head_slope)
                    else
                        zc=xc*params%tanthetac
                    endif
                    ...
                    do while (dabs(zdi-zdj).gt.ztoler)
                        ...
                        if((zi1-zj1).ge.0.or.(zi2-zj2).le.0)then
                            ...
                            print*, icount, zi1,zj1,zi2,zj2
                            print*, 'geometry%fix of node i and j are', geometry%fix(i),geometry%fix(j)
                            print*, 'node i and j area at:', geometry%x(i), geometry%y(i), geometry%x(j), geometry%y(j)
                            xd=l/2.d0
                            go to 334
                        endif
    ...
    334             continue
                    ...
                    if (geometry%zdiv(ndivide).lt.geometry%z(i).or.&
                        ...
                        print*,'bounding nodes coordinates are:', geometry%x(i), geometry%y(i), geometry%x(j), geometry%y(j)
                    endif

                !that i is lower than j
                endif
            ! end if for connections that are not part of the drainage network
            endif 
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ! update gemetry%chi and write deltachi to ASCII folder, YWANG
  ...
  do i=1,stack%nnode
     ...
     if (network%receiver(j).eq.0) then
        ...
        chi(j) = chi(k)+l*(Anot/(contributing_area(j)))**params%m
     endif
  enddo
  ! calculate deltachi
  deltachi1 = 0.d0
  deltachi2 = 0.d0
  do i=1,geometry%nnode
      deltachi2 = deltachi2+(chi(i)-geometry%chi(i))**2
      deltachi1 = deltachi1+abs(chi(i)-geometry%chi(i))
  enddo
  write(102,'(e15.7, e15.7, i10)') deltachi1, deltachi2, geometry%nnode
