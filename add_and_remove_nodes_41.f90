...
print*, 'number of iteration is: ', it
    !!!---------------------------- (2) ---------------------------- !!!
...
! Only internal nodes
  ...
  if ((params%move_points .or. params%jitter_nodes) .and. (params%buffer.eqv. .false.)) then
      ...
      min_dist =  min(params%max_adv*params%deltat,params%lmax) !if a distance of a node to the boundary < min_dist-->remove it      
...
print*, 'last check, number of iteration is: ', it
...
! this subroutine is to 
    ...
    if(ri.ne.0)then 
       if(geometry%z(i).ge.geometry%z(ri))then !connect to the lower receiver of crossed river
          ...
          if(p.ne.0)then
            if(geometry%z(i).ge.geometry%z(p))then!connect to the 1st receiver of the lower receiver of crossed river
                network%receiver(i)=p
                flagi1=1
                return        
            endif
            if(flagi1.eq.0)then
                q=network%receiver(p)
                if(q.ne.0)then
                    if(geometry%z(i).ge.geometry%z(q))then!connect to the 2nd receiver of the lower receiver of crossed river
                       network%receiver(i)=q
                       flagi2=1
                       return
                    endif
                endif
       ...
       if(flagi.eq.0 .and. flagi1.eq.0 .and. flagi2.eq.0)then ! can't find lower point to connect, make i a lake
    ...
    ! case2, crossed river nodes are neighbours of i receiver
    if(ri.eq.0 .and. rj.ne.0)then
        if(geometry%z(i).ge.geometry%z(rj))then
            ...
            if(w.ne.0)then
                if(geometry%z(i).ge.geometry%z(w))then
                    network%receiver(i)=w
                    flagj1=1
                    return       
                endif
                if(flagj1.eq.0)then
                    v=network%receiver(w)
                    if(v.ne.0)then
                        if(geometry%z(i).ge.geometry%z(v))then
                            network%receiver(i)=v
                            flagj2=1
                            return                            
                        endif                        
                    endif
        ...
        if(flagj.eq.0 .and. flagj1.eq.0 .and. flagj2.eq.0)then
          network%receiver(i)=0
          geometry%erosion_rate(i)=0.0d0
          geometry%sediment_flux(i) = 0.d0
          call VTK_debug(geometry,delaunay,params,network,stack,'artifactlakej_',params%istep)                              
        endif
    endif
