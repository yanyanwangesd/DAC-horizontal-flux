 ...
 do i=1,geometry%nnode
    if(geometry%fix(i).ge.1 )then
         ...
         if(  bcon.eq.2 .or. bcon.eq.10.  .or. bcon.eq.18  .or. bcon.eq.26   ) then
             ...
             !add lines to deal with corner nodes,corner nodes have no internal node neighbour
             ! make corner node to be average of boundary neigbours, @YWang, September2024
             if(ncount.eq.0)then
                 !find neighbours that are not boundaries
                 do k=1,geometry%nb(i)
                    j=geometry%nn(k,i)                   
