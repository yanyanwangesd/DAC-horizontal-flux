subroutine erode (geometry,network,stack,delaunay,params)
  ...
  ! add lines to write ascii file of erosion rate change per timestep  @YWANG, March 2022
  ...
  type (parm) params
  type (del) delaunay
  integer i,is,j,lakeflag,icount, jcount,dep_nnode, k, l, m, icond
  ...
  double precision umax, umin, vmax, vmin
  
  ! lines to write ascii file of erosion rate change per timestep  @YWANG, March 2022-------
  double precision, dimension(:),allocatable:: erodeit, zinit
  double precision deltae1, deltae2
  allocate(erodeit(geometry%nnode))
  allocate(zinit(geometry%nnode))
  !make a copy of old erosion rate
  do i = 1, geometry%nnode
      erodeit(i) = geometry%erosion_rate(i)
      zinit(i) = geometry%z(i)
  enddo
  ! lines to write ascii file of erosion rate change per timestep  @YWANG, March 2022-------
  ...
  do i=1,geometry%nnode        
       if(network%receiver(i).eq.0.and.geometry%fix(i).eq.0)then   !find lake nodes            
...
! write lake fill info to vtk, ---Feb 27,2025 YW 
  ...
  do i = 1, geometry%nnode
     if(network%receiver(i).ne.0.and.geometry%fix(i).eq.0)then
         if(geometry%y(i) .le. geometry%yl)then
            if(geometry%z(i).lt.geometry%z(network%receiver(i)))then
            ...
            if(geometry%z(i).eq.geometry%z(network%receiver(i)))then
                ...
                !print*,'node i and its reveiver are equal elevation at:', geometry%x(i), geometry%y(i), geometry%x(network%receiver(i)),geometry%y(network%receiver(i))
            endif
         endif
     endif
  enddo
...
!!$        if(j.ne.0.and.geometry%fix(i).eq.0)then
     ...
     do is=1,stack%nnode
        ...
        if (geometry%fix(i).eq.0 .and. network%receiver(i).ne.0) then
           ...
           if (geometry%z(i).lt.geometry%z(j)) then
              ...
              STOP 'error in erode.f90 -reciever higher than donor'
           endif
...
!!$           endif
  ...
  do i=1, geometry%nnode
     ...
     if(geometry%fix(i).ne.0 .and.(params%jitter_nodes .or. params%move_points))then
        geometry%erosion_rate(i)=geometry%w(i)*1.d0 ! make boundary uplift high to facilitate capture near boundary to stablize grid
     endif
  enddo
  ...
  deltae1 = 0.d0
  do i = 1, geometry%nnode
      deltae1 = deltae1 +abs(erodeit(i)-geometry%erosion_rate(i))
      deltae2 = deltae2+(erodeit(i)-geometry%erosion_rate(i))**2
  enddo
  write(101,'(e15.7, e15.7, i10)') deltae1, deltae2, geometry%nnode
  deallocate(erodeit)
  deallocate(zinit)
  ! lines to write ascii file of erosion rate change per timestep  @YWANG, March 2022-------
