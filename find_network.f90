subroutine find_network (geometry,delaunay,network,iflag)
  ...
  do i=1,geometry%nnode
     ...
     do j=1,geometry%nb(i)
        ...
        if (slope.gt.0.d0  .and. geometry%fix(i).eq.0    .and.  (geometry%y(i).gt.0.5d0 .and. geometry%y(i).lt.geometry%yl-0.5d0)) then
     ...
     enddo
     if(network%receiver(i).gt.0)then
      if(geometry%fix(i).gt.0 .and. geometry%fix(network%receiver(i) ).gt.0 )network%receiver(i)=0
      endif
  enddo
  ...
  do i=1,network%nnode
     ...
     if (k.ne.0) then
        ...
        if (network%ndon(k).gt.network%ndonmax   .and. ( .not. geometry%fix(k).ge.0 .and. geometry%fix(k).ge.0   )  ) stop 'network%ndonmax too small'
