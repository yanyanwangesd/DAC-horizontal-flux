subroutine find_order (geometry,network,stack,delaunay,params)
  ...
  do i=1,geometry%nnode
     if (network%receiver(i).eq.0.or.geometry%fix(i).ge.1) then
...
!            print*,'stop at time:', params%time
           STOP
        endif
     endif
  enddo
  ...
  if (stack%nnode.ne.geometry%nnode) then
     ...
     call VTK_debug (geometry,delaunay,params,network,stack,'findorder',params%istep)
     ...
     do i=1, geometry%nnode
        ...
        enddo
        print*,'missing node in stack', i, geometry%x(i), geometry%y(i)
111     continue
     enddo
     stop
  endif
