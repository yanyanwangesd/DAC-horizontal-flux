subroutine reinit_geometry (geometry,params,network,delaunay)
  ...
  ! @YWANG, FEB. 2022, don't give value here, should use the input from init.dac
 !geometry%nnmax=12 ! nnmax is the maximum number of neighbour nodes per node; 12 is usually enough
  ...
  allocate (geometry%chi(geometry%nnode_max))
  ...
  ! @YWANG, JAN. 2022
  allocate (geometry%card_flow(geometry%nnode_max), geometry%jitter_direction(geometry%nnode_max),geometry%jitter_flag(geometry%nnode_max,3),geometry%jitter_residual(geometry%nnode_max))
 ...
 enddo
  ...
  enddo
  do i = 1,geometry%ndivide
     read(51) geometry%nndivnode(i,1), geometry%nndivnode(i,2), geometry%nndivtri(i,1),geometry%nndivtri(i,2)
  enddo
  ...
  enddo
   ...
   ! @YWANG, JAN. 2022
  do i=1,geometry%nnode
     read(51) geometry%card_flow(i)
  enddo
  do i=1,geometry%nnode
     read(51) geometry%jitter_flag(i,1), geometry%jitter_flag(i,2), geometry%jitter_flag(i,3)
  enddo
  geometry%jitter_direction = 0
  do i=1,geometry%nnode
     read(51) geometry%jitter_residual(i)
  enddo
  geometry%jitter_residual = 0.d0
   ! @YWANG, JAN. 2022
   
  do i=1,geometry%nnode
      read(51) geometry%u(i)
  enddo
  
  do i=1,geometry%nnode
      read(51) geometry%v(i)
  enddo

  do i=1,geometry%nnode
      read(51) geometry%w(i)
  enddo
