subroutine find_hack (geometry,network,stack)
  ...
  ! store Hack data and Horton data to the ASCII folder, YWANG Aug. 2022
  open (40,file='ASCII/HackData',status='unknown')
  do i=1,geometry%nnode
     write(40,'(2i7,2f16.3,i3)') i, geometry%catchment(i), contributing_area(i), max_length(i), geometry%strahler(i)
  enddo
  close (40,err=1232)
  open (41,file='ASCII/HortonData',status='unknown')
  horton_length = 0.
  do i=stack%nnode,1,-1
      j=stack%order(i)
