subroutine add_remove_nodes(geometry,network,params,delaunay,stack)
  ...
  double precision buffery
  double precision xmaxinternal, ymaxinternal, xmininternal, ymininternal
  ...
  !when advecting nodes there are special add and remove operations (step 6+)for which the following is needed:
  if (params%buffer) buffery = geometry%yl+params%lmax*1.d0
  ...
  if (params%move_points) then
    ...
    do i=1,geometry%nnode
        if(geometry%fix(i) .ne.0)then
            xmin = min(xmin, geometry%x(i))
            xmax = max(xmax, geometry%x(i))
            ymin = min(ymin, geometry%y(i))
            ymax = max(ymax, geometry%y(i))
        else
            ymininternal = min(ymininternal, geometry%y(i))
            ymaxinternal = max(ymaxinternal, geometry%y(i))
            xmaxinternal = max(xmaxinternal, geometry%x(i))
            xmininternal = min(xmininternal, geometry%x(i))
        endif
    enddo     
    ! extra line to make it for buffer zone of fixed back boundary, @YWANG, March 2022
    if (params%buffer) ymax = min(ymax,buffery)
  endif
...
!   This section is to add nodes on channels where the channel connect two nodes that are not neighbors.
  ...
  !if(ncount.gt.0) call VTK_debug(geometry,delaunay,params,network,stack,'beforestopflipping_',params%istep)
  it=1
  do while (missing_node.eq.1)
      ...
      do i=1,nnold ! loop over the nodes
          if(geometry%fix(i).eq.0.and.network%receiver(i).ne.0)then ! deal with an internal non-lake node
               ...
               if(flag.eq.0)then !receiver is not a neighbor 
                  ...
                  if(flagcrossi.and.ri.gt.0)then
                    if(geometry%z(i).ge.geometry%z(ri))then
                  ...
                  print*, 'problem connnection is at:', geometry%x(i), geometry%y(i),geometry%x(j), geometry%y(j)
                  print*, 'add node to stop flipping at:', geometry%x(geometry%nnode), geometry%y(geometry%nnode)                 
               ...
               endif !endif cross river
               endif ! end if of non-neighbour receiver case
     ...
     !need to continuously update donors and receivers
      ...
      if (it.gt.50) then
          print*, 'missing_node is', missing_node
          print*, 'ncount is', ncount
          missing_node=0
          print*, 'time is', params%time
          print*,'Problem is at node i coordinates of', geometry%x(i),geometry%y(i) 
...
print*, 'ymax after stop flipping 1:', maxval(geometry%y(1:geometry%nnode))
            ...
            if(network%receiver(i).ne.j.and.network%receiver(j).ne.i) then ! check non-channel
               if (geometry%z(i).lt.geometry%z(j)) then !add to lowest node
                  ...
                  if (l.gt.params%ldivmax) then ! add node on the connection
                        ...
                        geometry%z(geometry%nnode)=(geometry%z(i)+geometry%z(j))/2.d0
                        print*, 'add node between', geometry%x(i), geometry%y(i), geometry%x(j), geometry%y(j)
                        print*, 'add node at:', geometry%x(geometry%nnode),  geometry%y(geometry%nnode)
  ...
  if(nadd_between_channel .gt. 0 .or. nadd_on_channel .gt. 0 .or. nadd_boundary .gt. 0)then
      ...
      call find_network (geometry,delaunay,network,1) ! find neighbour list only
      call check_neighbor_receiver (geometry,network,ncount)
      !print*, 'after adding nodes sections, the non-neighbour connections are:', ncount
      !if(ncount .gt. 0)then
...
!!!For that reason the donor receiver array needs to be updated
  if (nrem_small_area.gt.0.or.nadd_on_channel.gt.0&
    ...
    enddo
    print*, 'internal ymax before removing close to boundary:', ymaxinternal
    
  !!!---------------------------- (6) ---------------------------- !!!
! This section removes nodes that are too close to the boundary due to jitter or advection, but without a buffer zone
!!! routine to remove nodes close to boundary, is necessary when jitter nodes
! Only internal nodes
  nrem_close_to_boundary = 0
  if ((params%move_points .or. params%jitter_nodes) .and. (params%buffer.eqv. .false.)) then
...
557     if (i.le.geometry%nnode) then ! i can grow above geometry%nnode because geometry%nnode can become smaller duringt this loop
    ...
    enddo
    print*, 'internal ymax after removing close to boundary:', ymaxinternal

 !!!---------------------------- (6+) routine to deal with nodes close to boundary within buffer zone @YWANG ---------------------------- !!!
!  Special for advection: nodes advect towards boundary and getting too close if flux/adection out the boundary, 
!                         make a fake buffer zone to advect the boundary nodes to the buffer zone. Remove node if
!                         the node is advected out of the buffer zone. Note that this is only for advecting towards
!                         back boundary, YWANG, March 2022!!!!!!!!
  
  ! remove node that advects out of the buffer zone 
  if(params%move_points .and. params%buffer)then
      do i=1, geometry%nnode
556       if(i.le.geometry%nnode) then ! i can grow above geometry%nnode because geometry%nnode can become smaller during this loop
              if(abs(geometry%boundary(4)).eq.16 )then
                  if(geometry%y(i).ge.(buffery-2.d0))then

                      if(geometry%fix(i).eq.4 .and.geometry%x(i).ne.xmax.and.geometry%x(i).ne.xmin)then ! keep the back boundary and back corners at the buffer boundary
                          geometry%y(i) = buffery
                          geometry%x(i) = geometry%x(i)
                          geometry%fix(i)= 4
                      elseif(geometry%fix(i).eq.4 .and.(geometry%x(i).eq.xmax.or.geometry%x(i).eq.xmin))then
                          geometry%y(i) = buffery
                          geometry%x(i) = geometry%x(i)
                          geometry%fix(i)= 4
                      else
                          if((geometry%x(i).eq.xmin.and.geometry%y(i).ne.ymax.and.geometry%y(i).ne.ymin).or.&
                              &(geometry%x(i).eq.xmax.and.geometry%y(i).ne.ymax.and.geometry%y(i).ne.ymin).or.&
                              &(geometry%fix(i).eq.0))then !then it is internal or left/right boundary nodes
                              print*,'remove nodes close to boundary (in the buffer zone) routine is used'
                              geometry%nnode=geometry%nnode-1
                              k = network%receiver(i)
                              if (k.ne.0) then !i is not a lake
                                  do j = 1,network%ndon(i)
                                      h = network%donors(j,i) ! h is a donor of i
                                      network%receiver(h) = 0 ! now h is a donor of a lake
                                      geometry%z(h) = 0.d0
                                  enddo
                              else
                                  do j = 1,network%ndon(i)
                              ...
                              enddo
                              go to 556 !without increasing i becuase the node array is smaller.
                          endif
                      endif
  ...
  if(params%move_points .and. params%buffer)then
      vmax = maxval(geometry%v(1:geometry%nnode))
      do i=1, geometry%nnode
          if(abs(geometry%boundary(4)).eq.16 .and. params%buffer)then
              !if(geometry%y(i).gt.geometry%yl.and.geometry%y(i).lt.(geometry%yl+params%lmax/3))then
              if(geometry%y(i).gt.geometry%yl.and.params%time.ge.(buffery-geometry%yl)/vmax)then
                  if(network%ndon(i).ne.0.and.network%receiver(i).eq.0)then
                      geometry%z(i) = 0.d0
                  endif
              endif
              if(geometry%y(i).gt.(geometry%yl+params%lmax*0.7).and.params%time.ge.(buffery-geometry%yl)/vmax.and.geometry%fix(i).eq.0)then
!                   geometry%z(i) = 0.d0+(geometry%y(i)-(geometry%yl+params%lmax/3))*1.d-4
...
!-------------------------------------------------------------!
  ...
  !if (ncount.gt.0) call VTK_debug(geometry,delaunay,params,network,stack,'afterboundary',params%istep)
    ...
    enddo
    print*, 'internal ymax before stop flipping 2:', ymaxinternal
  ...
  do while (missing_node.eq.1)
    ...
    if (it.gt.50) then
      missing_node=0
      print*,'Problem is at node i coordinates of', geometry%x(i),geometry%y(i)
      print*, 'fix of i and its receiver', geometry%fix(i), geometry%fix(network%receiver(i))
      call VTK_debug(geometry,delaunay,params,network,stack,'lastcheck',params%istep)
      STOP 'more than 50 iterations in add_and_remove_nodes last check!!!'
    endif
    it = it+1
  enddo !while loop
    ...
    enddo
    print*, 'internal ymax after stop flipping 2:', ymaxinternal
