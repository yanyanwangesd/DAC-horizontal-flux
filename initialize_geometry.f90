...
zl =  geometry%zl
  ...
  ! add variable for cardinal_flow_direction and jitter, @YWANG
  allocate (geometry%card_flow(geometry%nnode_max), geometry%jitter_direction(geometry%nnode_max))
  allocate (geometry%jitter_flag(geometry%nnode_max,3))
  allocate (geometry%jitter_residual(geometry%nnode_max))


  geometry%card_flow = 0
  geometry%jitter_direction = 0
  !
  geometry%jitter_flag(:,1) = 2 !flag of whether to jitter at current step, 2(the node is never checked), 1(conidered, to jitter), 0(considered, not jitter)
  geometry%jitter_flag(:,2) = 0 !how many time steps the node has been jittered since Time 0
  geometry%jitter_flag(:,3) = 0 ! how many steps since the last time the node was jittered
  geometry%jitter_residual = 0.d0 ! residual jitter length due to hitting wall
