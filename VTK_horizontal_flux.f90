subroutine VTK_horizontal_flux(geometry,delaunay,params,network,stack, horizontal_flux, proj_area, label, step)
!
! used for debug, to output vtk files before or after specific operations on nodes
! the step is an integer, label is a string without any length limit, @YWANG, May 2023
  ...
  double precision, intent(in) :: horizontal_flux(geometry%nnode,2)
  double precision, intent(in) :: proj_area(geometry%nnode)
  double precision,dimension(:),allocatable:: ks
  double precision l, deltaz, deltachi

  allocate(ks(geometry%nnode))
  ...
  open (30,file='RUN5/DG_'//label//'_Landscape'//cs//'.vtk',status='unknown')
  open (31,file='RUN5/DG_'//label//'_rivers'//cs//'.vtk',status='unknown')
  open (32,file='RUN5/DG_'//label//'_divides'//cs//'.vtk',status='unknown')
...
!!!!!! @YW add lines to write Z-uplift rate (w), X-advection (u) ang Y-advection (v) file
  write(30,'(a)')'SCALARS uplift_rate float 1'
  write (30,'(a)')'LOOKUP_TABLE default'
  do i=1,geometry%nnode
  write(30,'(e12.4)') geometry%w(i)
  enddo

  write(30,'(a)')'SCALARS Xadvection_rate float 1'
  write (30,'(a)')'LOOKUP_TABLE default'
  do i=1,geometry%nnode
  write(30,'(e12.4)') geometry%u(i)
  enddo

  write(30,'(a)')'SCALARS Yadvection_rate float 1'
  write (30,'(a)')'LOOKUP_TABLE default'
  do i=1,geometry%nnode
  write(30,'(e12.4)') geometry%v(i)
  enddo

  write(30,'(a)')'SCALARS boundary integer 1'
  write (30,'(a)')'LOOKUP_TABLE default'
  do i=1,geometry%nnode
  write(30,'(I1)') geometry%fix(i)
  enddo
  
  write(30,'(a)')'SCALARS card_flow integer 1'
  write (30,'(a)')'LOOKUP_TABLE default'
  do i=1,geometry%nnode
  write(30,'(I10)') geometry%card_flow(i)
  enddo

  write(30,'(a)')'SCALARS receiver integer 1'
  write (30,'(a)')'LOOKUP_TABLE default'
  do i=1,geometry%nnode
  write(30,'(I10)') network%receiver(i)
  enddo

  write(30,'(a)')'SCALARS j_residual float 1'
  write (30,'(a)')'LOOKUP_TABLE default'
  do i=1,geometry%nnode
  write(30,'(e12.4)') geometry%jitter_residual(i)
  enddo
...
!!!!!! @YW add lines to write Z-uplift rate (w), X-advection (u) ang Y-advection (v) file
  ...
  write(30,'(a)')'SCALARS horizontal_flux float 1'
  write (30,'(a)')'LOOKUP_TABLE default'
  do i=1,geometry%nnode
     write(30,'(e12.4)') horizontal_flux(i,1)
  enddo 
 
  write(30,'(a)')'SCALARS vertical_flux float 1'
  write (30,'(a)')'LOOKUP_TABLE default'
  do i=1,geometry%nnode
     write(30,'(e12.4)') horizontal_flux(i,2)
  enddo 
  
  write(30,'(a)')'SCALARS prj_area float 1'
  write (30,'(a)')'LOOKUP_TABLE default'
  do i=1,geometry%nnode
  write(30,'(e12.4)') proj_area(i)
  enddo
  ...
  write(31,'(a)')'SCALARS chi float 1'
  write (31,'(a)')'LOOKUP_TABLE default'

  do i=1,geometry%nnode
     write(31,'(e12.4)') geometry%chi(i)
  enddo
  
  write(31,'(a)')'SCALARS ksn float 1'
  write (31,'(a)')'LOOKUP_TABLE default'
  do i=1,stack%nnode
     j=stack%order(i)
     if (network%receiver(j).eq.0) then
        ...
        deltachi = geometry%chi(j)-geometry%chi(k)
        deltaz = geometry%z(j)-geometry%z(k)
        if(deltachi.ne.0.d0 )then
            ks(j) = deltaz/deltachi
        else
            ks(j) = 0.d0
        endif        
     endif
  ...
  do i=1,geometry%nnode
     write(31,'(e12.4)') ks(i)
  enddo
!!!!!!!! Y.W. write morhozones
write(31,'(a)')'SCALARS morpho_zone float 1'
write (31,'(a)')'LOOKUP_TABLE default'
do i=1,geometry%nnode
    write(31,'(i1)') geometry%card_flow(i)    
enddo
