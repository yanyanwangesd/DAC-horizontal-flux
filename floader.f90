...
implicit none
  ...
  integer ia,s,sf, ci,numpol, nc,n,ib, iok,ic, nset,i
  real ra, r1, r2, r3,r4
...
params%istep=iread(s)                                  ! time step counter (should always be set to 0)
params%k_scalar1=dread(s)                      	! erodibility constant
params%n=iread(s)                              	! slope exponent in fluvial incision law
...
params%uplift_scalar2=dread(s)     ! uplift rate on boundary (used only for channel head calc)
params%advection_scalarx=dread(s)               ! uniform advection in the x direction
params%advection_scalary=dread(s)               ! uniform advection in the y direction
...
params%jitter_nodes=lread(s)                   ! flag to jitter all nodes horizontally each timestep
! @YWANG, JAN. 2022
params%jitter_range=dread(s)                    ! define how far node jitters laterally, between 0 and 1, recommend value magnitude 0.001
params%jitter_diffusivity = dread(s)            ! define the random walk diffusivity, recommend value range 0.0001-0.01
! @YWANG, JUN. 2022
...
params%sample_per_bin=INT(params%sample_per_bin/params%deltat) ! how many steps to store in bin which is decided by the input timespan, @YWANG, Dec. 2021
...
if(params%f_varies_with_xyz)then
  print*, 'activating params%f_varies_with_xyz'
